...
//define for RPY
 //  {'S', 'T', 
//              Roll_H, Roll_L, Pitch_H, Pitch_L, Yaw_H, Yaw_L,Reserve ,Reserve , 
//              Gx_H ,Gx_L ,Gy_H ,Gy_L ,Gz_H ,Gz_L ,Reserve ,Reserve , 
//              Ax_H, Ax_L, Ay_H, Ay_L, Az_H, Az_L ,difference, time_interval, 'E'}
#define RPY_PACKAGE_SIZE 27
...
typedef struct
{
    ...
    short gyro_[3] = {0};
    short accel[3] = {0};
    unsigned char rpy_package_[RPY_PACKAGE_SIZE] = {0};
    bool difference = 0;
    short old_rpy_[3] = {0};
    short old_gyro_[3] = {0};
...
//      imuLog += String(imu.az) + ", ";
  ...
  if (enableGyro) // If gyroscope logging is enabled
  {
    if ( enableCalculatedValues ) // If in calculated mode
    {
//      imuLog += String(imu.calcGyro(imu.gx)) + "gx, ";
//      imuLog += String(imu.calcGyro(imu.gy)) + "gy, ";
//      imuLog += String(imu.calcGyro(imu.gz)) + "gz, ";
      IMU.gyro_[0] = (IMU.old_gyro_[0] * LPF_Alpha ) + (imu.calcGyro(imu.gx) * (1 - LPF_Alpha) * (-1));
      IMU.gyro_[1] = (IMU.old_gyro_[1] * LPF_Alpha ) + (imu.calcGyro(imu.gy) * (1 - LPF_Alpha) * (-1));
      IMU.gyro_[2] = (IMU.old_gyro_[2] * LPF_Alpha ) + (imu.calcGyro(imu.gz) * (1 - LPF_Alpha) * (-1));
      IMU.old_gyro_[0] = IMU.gyro_[0];
      IMU.old_gyro_[1] = IMU.gyro_[1];
      IMU.old_gyro_[2] = IMU.gyro_[2];
//      imuLog += String(IMU.gyro_[0]) + "gx, ";
//      imuLog += String(IMU.gyro_[1]) + "gy, ";
//      imuLog += String(IMU.gyro_[2]) + "gz, ";
    }
    else
    {
...
//     ypr[i] = ypr[i];
     ...
     IMU.rpy_[2 - i] = (short)(ypr[i] * 100.0);
     if (IMU.rpy_[2 - i] < 0)
     {
         IMU.rpy_[2 - i] = abs(IMU.rpy_[2 - i]);
         IMU.rpy_[2 - i] |= 0x8000;
     }
  }

  
  if ((IMU.rpy_[0] == IMU.old_rpy_[0]) && (IMU.rpy_[1] == IMU.old_rpy_[1]) && (IMU.rpy_[2] == IMU.old_rpy_[2]))
      IMU.difference = false;
  else
      IMU.difference = true;
//calculate gyro
  for(int i = 0 ;i<3 ; i++)
  {
    if(IMU.gyro_[i] < 0)
    {
      IMU.gyro_[i] = abs(IMU.gyro_[i]);
      IMU.gyro_[i] |= 0x8000;
    }
  }

// calculate accel
//change the axis x and axis y and
    tmp_cel[0] = imu.calcAccel(imu.ay);
    tmp_cel[1] = imu.calcAccel(imu.ax) * (-1);
    tmp_cel[2] = imu.calcAccel(imu.az);

  for(int i = 0 ; i<3 ; i++)
  {
    IMU.accel[i] = (short)(tmp_cel[i] * 100);
    if (IMU.accel[i] < 0)
     {
         IMU.accel[i] = abs(IMU.accel[i]);
         IMU.accel[i] |= 0x8000;
     }
  }
...
//              Roll_H, Roll_L, Pitch_H, Pitch_L, Yaw_H, Yaw_L,Reserve ,Reserve , 
//              Gx_H ,Gx_L ,Gy_H ,Gy_L ,Gz_H ,Gz_L ,Reserve ,Reserve , 
//              Ax_H, Ax_L, Ay_H, Ay_L, Az_H, Az_L ,Reserve ,Reserve , 
//    difference, time_interval, 'E'}
  //Generate package
  IMU.rpy_package_[cnt++] = 0x53; //S
  IMU.rpy_package_[cnt++] = 0x54; //T
  //rpy
  for (int i = 0; i < 3; i++)
  {
//    Serial.print(IMU.rpy_[i]);
//    Serial.print(", ");
      IMU.old_rpy_[i] = IMU.rpy_[i];
      IMU.rpy_package_[cnt++] = ((IMU.rpy_[i]) >> 8);      //High_8_bits
      IMU.rpy_package_[cnt++] = ((IMU.rpy_[i]) << 8) >> 8; //Low_8_bits
  }
  IMU.rpy_package_[cnt++] = 0xAF;
  IMU.rpy_package_[cnt++] = 0xAF;
  //gyro
  for (int i = 0; i < 3; i++)
  {
      IMU.rpy_package_[cnt++] = ((IMU.gyro_[i]) >> 8);      //High_8_bits
      IMU.rpy_package_[cnt++] = ((IMU.gyro_[i]) << 8) >> 8; //Low_8_bits
  }
  IMU.rpy_package_[cnt++] = 0xCF;
  IMU.rpy_package_[cnt++] = 0xCF;
  for(int i = 0; i < 3; i++)
  {
      IMU.rpy_package_[cnt++] = ((IMU.accel[i]) >> 8);      //High_8_bits
      IMU.rpy_package_[cnt++] = ((IMU.accel[i]) << 8) >> 8; //Low_8_bits 
  }
  IMU.rpy_package_[cnt++] = (unsigned char)(IMU.difference);          // difference
  IMU.rpy_package_[cnt++] = (unsigned char)timer_end - timer_start; // time_interval
  IMU.rpy_package_[cnt] = 0x45;   //E



//  if (enableSerialLogging)  // If serial port logging is enabled
//    LOG_PORT.print(imuLog); // Print log line to serial port
    //erver 10 ms send package
    if ((timer_end - timer_start) >= 10)
    {  
      for (int i = 0; i < RPY_PACKAGE_SIZE; i++)
      {
        //output for hardware
        LOG_PORT.write(IMU.rpy_package_[i]);

        //output for terminal check
//        LOG_PORT.print(IMU.rpy_package_[i], 16);
//        LOG_PORT.print(", ");
      }
//      LOG_PORT.print("\n");
  
      //for terminal check IMU data
//      LOG_PORT.print(String(ypr[2])+ "R  " );
//      LOG_PORT.print(String(ypr[1])+ "P  " );
//      LOG_PORT.println(String(ypr[0])+ "Y  " );
